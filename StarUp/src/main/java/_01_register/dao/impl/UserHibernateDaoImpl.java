...
public class UserHibernateDaoImpl implements UserDao {
	...
	public boolean nicknameExists(String nickname) {
		...
		try {
			...
			List<UserBean> list = session.createQuery(hql).setParameter("nickname", nickname).getResultList();
			if(list.isEmpty()) {
				exist = false;
			}else {
				exist = true;
	...
	// 檢查使用者在登入時輸入的帳號與密碼是否正確。如果正確，傳回該帳號所對應的MemberBean物件，
		...
		public UserBean checkIDPassword(String account, String password) {
			UserBean mb = null;
			String sql = "SELECT * FROM User m WHERE m.Account = ? and m.password = ?";
			try (
		...
		public UserBean getUser(int pk) {
			...
			try {
				tx = session.beginTransaction();
				List<UserBean> list = session.createQuery(hql).setParameter("uid", pk).getResultList();
